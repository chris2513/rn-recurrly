...
Where possible, add calls for PostHog's `identify()` function on the client side upon events like logins and signups. Use the contents of login and signup forms to identify users on submit. On the client, call `identify()` at login/signup. If you have server-side code, pass the client session/distinct ID (for example via `X-POSTHOG-DISTINCT-ID` and `X-POSTHOG-SESSION-ID` headers) to the server and ensure server-side events include that distinct ID so events can be correlated. Only perform the server-side step if there is server-side code.
